      ...
      <aside className="hidden md:flex w-60 border-r border-white/10 bg-[#020202] flex-col shrink-0">
        <div className="h-16 flex items-center px-5 border-b border-white/10">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded bg-white/10 flex items-center justify-center border border-white/10 text-zinc-200 font-bold text-sm">
              P
            </div>
          ...
          {navItems.map((item) => (
            <Link key={item.href} href={item.href}>
              <span className={`flex items-center gap-3 px-3 py-2 text-sm rounded-md transition-all duration-150 cursor-pointer ${
                isActive(item.href)
                  ? "bg-white/10 text-zinc-100 font-medium border-l-2 border-zinc-300 pl-[10px]"
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="md:hidden h-14 border-b border-white/10 flex items-center justify-between px-4 bg-black/90 backdrop-blur sticky top-0 z-10">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded bg-white/10 flex items-center justify-center border border-white/10 text-zinc-200 font-bold text-xs">
              P
            </div>
